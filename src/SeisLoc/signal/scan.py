...
#            self.output.write_scan(daten[:-1],CoaVp[:-1],dcoord[:-1,:])
    ...
    def _ErrorEllipse(self,COA3D):
        ...
        # If outside the region fit maximums to region
        if xmin < 0:
            xmin = 0
        if ymin < 0:
            ymin = 0
        ...
        if xmax > COA3D.shape[0]:
            xmax = COA3D.shape[0]
        if ymax > COA3D.shape[1]:
            ymax = COA3D.shape[1]
        if zmax > COA3D.shape[2]:
            zmax = COA3D.shape[2]


        # And calculate covariance matrix:

        indx = np.where((lx[xmin]*self.lookup_table.cell_size[0]) <= x_samples  <= (lx[xmax]*self.lookup_table.cell_size[0]) & (ly[ymin]*self.lookup_table.cell_size[1]) <= y_samples  <= (ly[ymax]*self.lookup_table.cell_size[1]) & (lz[zmin]*self.lookup_table.cell_size[2]) <= z_samples  <= (lz[zmax]*self.lookup_table.cell_size[2]))
        SumSW = samples_weights[indx]

        x_expect = np.sum(samples_weights[indx]*x_samples[indx])/SumSW
        y_expect = np.sum(samples_weights[indx]*y_samples[indx])/SumSW
        z_expect = np.sum(samples_weights[indx]*z_samples[indx])/SumSW
